...
class ArtistData:
    ...
    #     return self._info
    #
    # temp_variable = require_detail(info)
    # temp_variable(self)
    #
    # 1. 일반함수는 그냥 자신의 정보를 return해주는 역할
    # 2. 그런데 데코레이터를 쓰면 일반함수를 수정하지 않고 여러 기능을 추가
    # 3. 즉, 자신의 정보를 return해주기 전에, self.detail을 체크 한 뒤, 없으면 self.get_detail()실행
    # 4. 그리고 난 뒤, 자신의 정보 return
